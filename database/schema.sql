-- MyDiary Secure - Schéma de base de données Supabase
-- Version: 2.0.0 avec Supabase
...
CREATE INDEX idx_diary_entries_mood ON public.diary_entries(mood);
...
CREATE INDEX idx_backups_expires_at ON public.backups(expires_at);
...
BEGIN
    ...
    -- TODO: Calculer la série d'écriture (logique complexe)
    current_streak := 0;
    longest_streak := 0;
    ...
    INSERT INTO public.user_stats (
        ...
        target_user_id, entry_count, current_streak, longest_streak,
        common_mood, month_count, year_count,
        last_entry, NOW()
    )
    ON CONFLICT (user_id) DO UPDATE SET
        total_entries = EXCLUDED.total_entries,
        writing_streak = EXCLUDED.writing_streak,
...
-- Trigger pour recalculer les stats après insertion/modification d'entrée
CREATE OR REPLACE FUNCTION trigger_calculate_stats()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER diary_entries_stats_trigger
    AFTER INSERT OR UPDATE OR DELETE ON public.diary_entries
    FOR EACH ROW EXECUTE FUNCTION trigger_calculate_stats();
...
COMMENT ON COLUMN public.diary_entries.encrypted_content IS 'Contenu chiffré côté client avec AES-GCM';
COMMENT ON COLUMN public.diary_entries.encryption_salt IS 'Salt unique pour le chiffrement de cette entrée';
COMMENT ON COLUMN public.profiles.preferences IS 'Préférences utilisateur en JSON';